...
export function checkSecrets(changes: FileChanges): CheckResult[] {
  ...
  if (lowerName.includes('test') || lowerName.includes('spec') || lowerName.includes('mock') || lowerName.includes('fixture') || lowerName.includes('example') || lowerName.endsWith('.md')) return [];
  const results: CheckResult[] = [];
  changes.addedLines.forEach((line, idx) => {
    const trimmed = line.trim();
...
export function checkTestExists(changes: FileChanges, allFiles: Set<string>): CheckResult[] {
...
export function checkComplexity(changes: FileChanges, threshold: number): CheckResult[] {
  const ext = '.' + changes.filename.split('.').pop();
