...
function formatResults(results: CheckResult[]): string {
  if (results.length === 0) return '✓ All checks passed — ready to review!';
...
Returns a summary of issues with file paths and line numbers.`,
    inputSchema: {
      repo_path: z.string().describe(
        'Absolute path to the git repository root. Use the workspace root when checking changes before opening a PR.'
      ),
      base_sha: z.string().optional().describe(
        'Base commit SHA for diff (optional). If provided with head_sha, diffs between two commits instead of staged/unstaged.'
      ),
      head_sha: z.string().optional().describe(
        'Head commit SHA for diff (optional). Used with base_sha.'
      ),
      complexity_threshold: z.number().optional().describe(
        'Cyclomatic complexity threshold. Flag functions exceeding this branch count. Default: 10.'
      ),
    },
    annotations: {
      readOnlyHint: true,
...
Note: test coverage check requires knowing all project files, so it's skipped in this mode.`,
    inputSchema: {
      content: z.string().describe(
        'The code content to check. Can be a full file or a snippet.'
      ),
      filename: z.string().describe(
        'The filename (e.g., "src/auth.ts") — used to apply language-specific rules and detect if it is a test file.'
      ),
      complexity_threshold: z.number().optional().describe(
